...
WITH day_all AS (
    SELECT
        ...
        CHAIN,
        SUM(ACTUALGASCOST) AS ACTUALGASCOST_ALL
...
day_eip AS (
    SELECT
        ...
        u.CHAIN,
        SUM(u.ACTUALGASCOST) AS ACTUALGASCOST_EIP
    FROM BUNDLEBEAR.DBT_KOFI.ERC4337_ALL_USEROPS u
    INNER JOIN BUNDLEBEAR.DBT_KOFI.EIP7702_STATE_BASE s
        ON u.SENDER = s.AUTHORITY
        AND u.CHAIN = s.CHAIN
        AND DATE_TRUNC('day', u.BLOCK_TIME) >= s.START_DAY
        AND (s.END_DAY IS NULL OR DATE_TRUNC('day', u.BLOCK_TIME) <= s.END_DAY)
        AND s.IS_SMART_WALLET = TRUE
    WHERE
        {% if is_incremental() %}
        u.BLOCK_TIME >= DATEADD(DAY, -3, CURRENT_DATE())
        {% else %}
        u.BLOCK_TIME >= DATE('2025-05-05')
        {% endif %}
...
day_metrics AS (
    SELECT
        ...
        a.CHAIN,
        'day' AS TIMEFRAME,
        100 * COALESCE(e.ACTUALGASCOST_EIP, 0) / NULLIF(a.ACTUALGASCOST_ALL, 0)
            AS PCT_EIP7702_ACTUALGASCOST
    FROM day_all a
    LEFT JOIN day_eip e
        ON a.DATE = e.DATE
        AND a.CHAIN = e.CHAIN
    ...
    SELECT
        ...
        'all' AS CHAIN,
        'day' AS TIMEFRAME,
        100 * COALESCE(SUM(e.ACTUALGASCOST_EIP), 0) / NULLIF(SUM(a.ACTUALGASCOST_ALL), 0)
            AS PCT_EIP7702_ACTUALGASCOST
    FROM day_all a
    LEFT JOIN day_eip e
        ON a.DATE = e.DATE
...
week_eip AS (
    ...
    WHERE
        {% if is_incremental() %}
        u.BLOCK_TIME >= DATE_TRUNC('week', CURRENT_DATE()) - INTERVAL '1 week'
        {% else %}
        u.BLOCK_TIME >= DATE('2025-05-05')
        {% endif %}
        AND u.BLOCK_TIME < DATE_TRUNC('week', CURRENT_DATE())
    GROUP BY 1,2
),
...
month_eip AS (
    ...
    WHERE
        {% if is_incremental() %}
        u.BLOCK_TIME >= DATE_TRUNC('month', CURRENT_DATE()) - INTERVAL '1 month'
        {% else %}
        u.BLOCK_TIME >= DATE('2025-05-05')
        {% endif %}
        AND u.BLOCK_TIME < DATE_TRUNC('month', CURRENT_DATE())
    GROUP BY 1,2
),
