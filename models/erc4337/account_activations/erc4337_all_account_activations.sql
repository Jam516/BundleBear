{{ config
(
    materialized = 'table',
    copy_grants=true
)
}}

SELECT
    DATE_TRUNC('day', u.BLOCK_TIME) AS DATE,
    u.CHAIN,
    CASE 
        WHEN d.ACCOUNT_ADDRESS IS NOT NULL AND FACTORY_NAME != 'Unknown'
            THEN 'erc4337 factory - ' || d.FACTORY_NAME
        WHEN d.ACCOUNT_ADDRESS IS NOT NULL AND FACTORY_NAME = 'Unknown'
            THEN 'erc4337 factory - ' || d.FACTORY
        WHEN u.CALLED_CONTRACT IN (
            '0xa581c4a4db7175302464ff3c06380bc3270b4037',
            '0x75cf11467937ce3f2f357ce24ffc3dbf8fd5c226'
        ) THEN 'Safe4337Module'
        WHEN a.OP_HASH IS NOT NULL THEN 'eip7702 - ' || COALESCE(al.NAME, a.AUTHORIZED_CONTRACT)
        ELSE 'other factory'
    END AS PROVIDER,
    u.SENDER
FROM BUNDLEBEAR.DBT_KOFI.ERC4337_ALL_USEROPS u
LEFT JOIN BUNDLEBEAR.DBT_KOFI.ERC4337_ALL_ACCOUNT_DEPLOYMENTS d
    ON u.SENDER = d.ACCOUNT_ADDRESS
    AND u.CHAIN = d.CHAIN
LEFT JOIN BUNDLEBEAR.DBT_KOFI.EIP7702_ACTIONS a
    ON a.OP_HASH = u.OP_HASH 
    AND a.CHAIN = u.CHAIN
LEFT JOIN BUNDLEBEAR.DBT_KOFI.EIP7702_LABELS_AUTHORIZED_CONTRACTS al
    ON al.ADDRESS = a.AUTHORIZED_CONTRACT
QUALIFY ROW_NUMBER() OVER (PARTITION BY u.CHAIN, u.SENDER ORDER BY u.BLOCK_TIME ASC) = 1