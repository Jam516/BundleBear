...
with handleop AS (
    SELECT
        ...
        CASE WHEN t.RECEIPT_EFFECTIVE_GAS_PRICE = 0 THEN 0 
        ELSE (t.RECEIPT_L1_FEE + (t.RECEIPT_GAS_USED*t.RECEIPT_EFFECTIVE_GAS_PRICE)) / 1e18
        END as bundler_outflow
    FROM {{ source('base_raw', 'transactions') }} t
    INNER JOIN {{ source('base_raw', 'traces') }} tr 
