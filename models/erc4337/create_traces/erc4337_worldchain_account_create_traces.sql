...
WITH activated_accounts AS (
    SELECT DISTINCT SENDER FROM BUNDLEBEAR.DBT_KOFI.ERC4337_WORLDCHAIN_USEROPS
)
...
FROM WORLDCHAIN.RAW.CONTRACTS c
...
{% if is_incremental() %}
WHERE c.BLOCK_TIMESTAMP >= CURRENT_TIMESTAMP() - interval '3 day'
{% endif %}
