...
with op as (
    with base as (
        SELECT 
            ...
            , op.executeCall
        FROM {{ ref('erc4337_arbitrum_nova_entrypoint_call_innerhandleop') }} op
        INNER JOIN {{ source('arbitrum_nova_raw', 'transactions') }} tx 
    ...
    , joined as (
        ...
        FROM base b
        INNER JOIN {{ source('arbitrum_nova_raw', 'traces') }} t 
...
SELECT 
    ...
    , case 
        ...
        when (INPUT = '0x' AND common.udfs.js_hextoint_secure(SUBSTRING(executeCall, 75, 64))::BIGINT/1e18 > 0) then 'eth_transfer'
