...
with op as (
    with base as (
        SELECT 
            ...
            , op.executeCall
        FROM {{ ref('erc4337_avalanche_entrypoint_call_innerhandleop') }} op
        INNER JOIN {{ source('avalanche_raw', 'transactions') }} tx 
    ...
    , joined as (
        ...
        FROM base b
        INNER JOIN {{ source('avalanche_raw', 'traces') }} t 
...
SELECT 
    ...
    , case 
        ...
        when (INPUT = '0x' AND common.udfs.js_hextoint_secure(SUBSTRING(executeCall, 75, 64))::BIGINT/1e18 > 0) then 'direct_transfer'
        else 'empty_call' 
        end as called_contract
    , case 
        when INPUT != '0x' then COALESCE(TEXT_SIGNATURE_SHORT, LEFT(INPUT,10))
        when (INPUT = '0x' AND common.udfs.js_hextoint_secure(SUBSTRING(executeCall, 75, 64))::BIGINT/1e18 > 0) then 'avax_transfer'
    ...
    , case when INPUT != '0x' then 0
      else common.udfs.js_hextoint_secure(SUBSTRING(executeCall, 75, 64))::BIGINT/1e18 
      end as value
FROM op 
