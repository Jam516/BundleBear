...
with op as (
    with base as (
        SELECT 
            ...
            , op.executeCall
        FROM {{ ref('erc4337_linea_entrypoint_call_innerhandleop') }} op
        INNER JOIN {{ source('linea_raw', 'transactions') }} tx 
    ...
    , joined as (
        ...
        FROM base b
        INNER JOIN {{ source('linea_raw', 'traces') }} t 
...
SELECT 
    ...
    , case 
        ...
        when (INPUT = '0x' AND common.udfs.js_hextoint_secure(SUBSTRING(executeCall, 75, 64))::BIGINT/1e18 > 0) then 'matic_transfer'
